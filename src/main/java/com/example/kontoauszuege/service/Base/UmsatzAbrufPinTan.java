...
{
    private final static String TAN_MEDIA_CODE = "982";
    ...
    static String UebSepa(BankInfo info, HBCIHandler handle, Konto src, Contact contact) throws Exception {

        //var umsatzJob = new GVUebSEPA(handle);

        //HBCIJob umsatzJob = handle.newJob("UebSepa");//Camt
        ...
        if( !HBCIUtils.checkIBANCRC(contact.dstIban))
        {
            throw new Exception(String.format("CRC Check fehlgeschlagen für die iban:'%s'. Empfänger Name: %s" ,
                    contact.dstIban, contact.dstName));
        }

        dst.iban = contact.dstIban;
        dst.bic = contact.dstBic;
        dst.name = contact.dstName;
        umsatzJob.setParam("dst",dst);
        umsatzJob.setParam("btg",new Value(contact.btgValue,"EUR"));

        if (contact.usage != null && contact.usage.length() > 0)
            umsatzJob.setParam("usage",contact.usage);

        String endToEndId = contact.endToEndId;
    ...
    public static AbrufResult Start(Contact contact) throws Exception
    {
        ...
        System.out.println(String.format("!Starte %s:  name='%s' blz=%s user=%s startdate=%s",contact.op, contact.name, contact.blz, contact.user, format.format(contact.startdate)));
        ...
        try
        {
            ...
            if(contact.op.equals("Kl")) {
                var ret = new AbrufResult();
                ret.kontenListe = Arrays.stream(konten).map(x->x.iban).toList();
                return ret;
            }
            ...
            Konto k = null;
            String kontoNr = contact.iban;

            for (int i = 0; i < konten.length; i++) {
                log(" Konto: " + konten[i].number);
                if(konten[i].iban.equals(kontoNr))
                {
                    k = konten[i];
                }
            }
            ...
            if(k == null)
            {
                throw new Exception(String.format("Konto nicht gefunden: %s" , kontoNr));
            }
/*
...
*/
            ...
            if(contact.op.equals("Ueb")) {
                var ret = new AbrufResult();
                ret.uebResult = UebSepa(info, handle, k, contact);//UebSEPA
                return ret;
            }

            HBCIJob umsatzJob = handle.newJob("KUmsAllCamt");//Camt

            Date myDate = contact.startdate;
            ...
            var iban = HBCIUtils.getIBANForKonto(k);
...
*/
            ...
            //String json = gson.toJson(kblist);
        ...
        {
            System.out.println("------------- !!! Kontoauszüge holen mit FEHLER beendet!!!  ---------------: "+ex.toString());
            throw ex;
        }
        finally
        {
